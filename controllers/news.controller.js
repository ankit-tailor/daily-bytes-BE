...
const { getAllNewsService, addNewsArticleService, updateNewsArticleService, deleteNewsArticleService, getNewsArticleByIdService } = require('../services/news.services')
...
const getNewsArticleByIdCtrl = async (req, res) => {
    try {
        ...
        if (!res) {
            res.status(400).json({ error: "Failed to update news article, something went wrong!!!" })
        } else {
            res.json(result)
        }
    } catch (err) {
...
const addNewsArticleCtrl = async (req, res) => {
    try {
        ...
        if (!res) {
            ...
            res.status(404).json({ error: "Failed to add new news article, something went wrong!!" });
        } else {
            console.log(result, "INSIDE ELSE")
            res.json(result)
        }
...
const deleteNewsArticleCtrl = async (req, res) => {
    try {
        ...
        if (!res) {
            res.status(400).json({ error: "Failed to delete news article, something went wrong!!!" })
        } else {
            res.json(result)
        }
    } catch (err) {
